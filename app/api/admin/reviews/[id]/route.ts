import { NextRequest, NextResponse } from 'next/server';
export const dynamic = 'force-dynamic';
import { approveReview, deleteReview } from '@/lib/db';
import { logAudit } from '@/lib/audit';

export async function PATCH(
  _request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    const id = parseInt(params.id, 10);
    if (isNaN(id)) return NextResponse.json({ error: 'Неверный ID' }, { status: 400 });
    const ok = approveReview(id);
    if (!ok) return NextResponse.json({ error: 'Не найдено' }, { status: 404 });
    await logAudit('approve', 'review', params.id, _request.headers.get('x-forwarded-for') ?? undefined);
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: 'Ошибка' }, { status: 500 });
  }
}

export async function DELETE(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    const id = parseInt(params.id, 10);
    if (isNaN(id)) return NextResponse.json({ error: 'Неверный ID' }, { status: 400 });
    const ok = deleteReview(id);
    if (!ok) return NextResponse.json({ error: 'Не найдено' }, { status: 404 });
    await logAudit('delete', 'review', params.id, request.headers.get('x-forwarded-for') ?? undefined);
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: 'Ошибка' }, { status: 500 });
  }
}
